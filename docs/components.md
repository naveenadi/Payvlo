...
<script lang="ts">
  ...
  $: error = validationResult?.is_valid === false 
...
<script lang="ts">
  ...
  const schema = z.object({
    ...
    gstin: z.string().regex(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[Z]{1}[0-9A-Z]{1}$/, 'Invalid GSTIN format'),
...
.cgst-amount { @apply text-blue-600; }
.sgst-amount { @apply text-green-600; }
.igst-amount { @apply text-orange-600; }
```
